...
if __name__ == '__main__':
    ...
    # dropout_rate_wrn - wide resnet dropout
    ...
    parser.add_argument('--load', type=str, default='./experiments/exp_1', help='load path')
    # default is false.
    ...
    # this maybe different for both the optimizers ()
    ...
    if opt.optimizer=='SGD':
        optimizer = torch.optim.SGD([{'params': cls_head.parameters()}], lr=0.001, momentum=0.9, \
                                              weight_decay=5e-4, nesterov=True)

    else:
        optimizer = torch.optim.Adam([{'params': cls_head.parameters()}], lr=opt.learn_rate)
