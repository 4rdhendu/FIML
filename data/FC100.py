...
#_FC100_DATASET_DIR = '/scratch_net/minnow_second/data/few-shot/FC100'
_FC100_DATASET_DIR = '/home/ardhendu/Data/MetaOptNet/FC100'
#_FC100_DATASET_DIR = '/home/ubuntu/data/MetaOptNet/Datasets/FC100'

# this is where the label is changed to the relative indexes
...
class FC100(data.Dataset):
    def __init__(self, phase='train', do_not_use_random_transf=False):
        ...
        if self.phase=='train':
            ...
            # TODO: labels ka sara khel samjho
            ...
            self.Kall_lst = torch.tensor(self.labelIds_base)
...
class FewShotDataloader():
    ...
    def get_abs_class(self, Kall, Ye):

        # need to change stuff here
        if self.dataset.phase == 'train':
            Kall_lst = self.dataset.Kall_lst
            # Kall_lst = torch.tensor(self.dataset.labelIds_base) # ye baar baar kyun hi karna hai (ek hi baar dataset declare ho wahin pe kar do bas)

            Kabs = torch.zeros(len(Kall))
            for i in range(len(Kall)):
                Kabs[i] = (Kall_lst == Kall[i]).nonzero()
            Kfin = Kabs[Ye]
            return Kfin
        else:
            return torch.zeros(len(Ye))
    ...
    def get_dataloader(self):
        def load_function(iter_idx):
            ...
            if len(Exemplars) > 0:
                Xe, Ye = self.createExamplesTensorData(Exemplars)
                Kabs = self.get_abs_class(Kall, Ye)
                return Xe, Ye, Xt, Yt, Kall, nKbase, Kabs
            else:
                return Xt, Yt, Kall, nKbase
